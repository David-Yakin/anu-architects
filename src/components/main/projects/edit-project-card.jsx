...
import { getProject, editProject, editprojectWithPics } from '../../../services/projectService';
    ...
    async componentDidMount(){
     ...
     upload = ( data) => {
      ...
      doSubmit = async ()=>{
        ...
        const projectId = this.props.match.params.id; 
        const { data } = await getProject(projectId);
        const {cardUrl, urlPamorama, urlBefore, urlSketch, urlImaging, urlConstraction, urlGallery} = dataFromState
      
        try{
            if(cardUrl === undefined && urlPamorama === undefined && urlBefore === undefined && urlSketch === undefined && urlImaging === undefined && urlConstraction === undefined && urlGallery === undefined ){
                    ...
                    <div className="center">
                        <form className='col-10 bg-light rounded mb-4 pt-2' 
                         ...
                         method='POST'>
                            ...
                            { this.renderInput('year', ' השנה בה הסתיים הפרויקט *', true, 'number' ) }
                            ...
                            { this.renderFileInput('urlBefore', 'התמונה של הנכס לפני הבניה/ השיפוץ' ) }
                            ...
                            { this.renderFileInput('urlConstraction', 'תמונה מהשיפוצים / הבנייה של הפרויקט' ) }
