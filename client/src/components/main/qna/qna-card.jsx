...
const QnaCard = ({ item, toggleQna, index, handleQnaDelete }) => {
  ...
  return (
    <div className="accordion">
      <div className="card-header" onClick={() => toggleQna(index)}>
        <h2 className="mb-0 text-right display-4">{question}</h2>
      </div>

      <div className={item.open ? "qna-open m-0" : "qna-close m-0"}>
        <div className="card-body row">
          <div className="card-text col-12  text-right">
            <h2 className="card-title h2Title">{answer.title}</h2>
            ...
            {user && user.isAdmin && (
              <span>
                <Link
                  to={`/private-area/edit-qna-card/${item._id}`}
                  className="far fa-edit text-dark   text-decoration-none"></Link>
                <span> | </span>
                <a
                  href="/"
                  onClick={e => {
                    handleQnaDelete(item._id, e);
                  }}
