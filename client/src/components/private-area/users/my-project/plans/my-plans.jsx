...
class MyPlans extends Form {
  ...
  async componentDidMount() {
    ...
    let images = data.images.plans;
    const counter = images.length - 1;
    this.setState({ images, counter, project: data });
  }

  schema = {
    plans: Joi.string().required().min(2).max(255).label("plans"),
  };
    ...
    try {
      const remarks = images.filter(image => {
        if (image._id === images[counter]._id) {
          image.remarks = data.plans;
          return image;
        }
        return image;
      });

      this.setState({ images: remarks, data: { plans: "" } });
      await editImage(projectId, image, "edit-plans");
  ...
  onArrowClick(symbol) {
    ...
    this.setState({ counter, data: { plans: "" } });
  }
  ...
  handleImageDelete = async (imageID, e) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const counter = images.length - 1;
        deleteImage(images, projectId, "delete-plans");
  ...
  generageCard() {
    ...
    return "אין תמונות תוכניות ארכיטקטיות במאגר המידע";
  }
