...
class UploadSketches extends Form {
  ...
  doSubmit = async () => {
    try {
      ...
      await uploadImage(image, projectId, "UploadSketches");
      toast("הפרויקט עודכן בהצלחה!");
      return this.props.history.replace(
        `/private-area/project/sketches/${this.props.match.params.id}`
      );
    } catch (error) {
      ...
      <div className="container-fluid sign-in">
        <div className="center ">
          <form
            ...
            {this.renderInput("imageAlt", "תאר את הסקיצה")}

            <div className="center">
              <Link
                className="a-herf mb-2 text-rtl"
                to={`/private-area/project/sketches/${this.props.match.params.id}`}>
                {" "}
                התחרטת?
