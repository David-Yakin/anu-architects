...
class MyGallery extends Form {
  ...
  async componentDidMount() {
    ...
    let slider = data.images.gallery;
  ...
  doSubmit = async () => {
    try {
      ...
      await uploadImage(image, projectId, "uploadGallery");
  ...
  handleImageDelete = async (imageID, e) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const projectId = this.props.match.params.id;
        const counter = slider.length - 1;
        deleteImage(slider, projectId, "delete-gallery");
