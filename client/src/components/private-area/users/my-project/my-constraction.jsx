...
class MyConstraction extends Form {
  state = {
  ...
  async componentDidMount() {
    ...
    let slider = data.images.constraction;
  ...
  doSubmit = async () => {
    try {
      ...
      await uploadImage(image, projectId, "uploadConstraction");
  ...
  handleImageDelete = async (imageID, e) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        const projectId = this.props.match.params.id;
        const counter = slider.length - 1;
        deleteImage(slider, projectId, "delete-constraction");
  ...
  generageCard() {
    ...
    return "אין תמונות מהבניה או השיפוץ במאגר המידע";
  }
  generateForm() {
    return (
  ...
  render() {
    ...
    if (user && user.isAdmin | (user._id === project.userID))
      return (
        <div id="theProcess" className=" container-fluid">
          <Titles
            titleBold="תמונות"
            title="מהבניה והשיפוץ"
            subTitle={`כאן תוכל לראות ${
              user.isAdmin ? "להוסיף או למחוק" : ""
            } תמונות מהבניה והשיפוץ של הפרויקט ${project.name}`}
          />
    ...
    return "אין תמונות מהשיפוצים";
  }
}

export default MyConstraction;
